...
const char* serverAddress = "192.168.42.223";  // The domain address or IP address of the server
const int serverPort = 8000;                      // port number the server listens on
const String endpoint = "http://192.168.42.223:8000/receive_data/";                // URL where you want to send the JSON data
...
void setup() {
  ...
  pinMode(relay_pin, OUTPUT);
   ...
   // Define the endpoint to handle JSON data
...
if (client.connect(serverAddress, serverPort)) {
    ...
    client.println("POST " + endpoint.substring(endpoint.indexOf("/", 7)) + " HTTP/1.1");
